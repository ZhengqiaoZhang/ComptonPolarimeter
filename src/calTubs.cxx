...
//_____________________________________________________________________________
calTubs::calTubs(const G4String& nam, G4double x, G4double y, G4double z, G4double xysize, G4double zsize,G4double rotation,G4double color1,G4double color2,G4double color3,G4LogicalVolume *top): Detector(),
  G4VSensitiveDetector(nam), fNam(nam) {

  G4cout << "  calTubs: " << fNam << G4endl;

  //detector shape
  //G4double xysize = 2000*mm;
  G4Tubs *shape = new G4Tubs(fNam, 0*cm,xysize*0.5, zsize/2., 0, 360*deg);
...
G4RotationMatrix* rot_y = new G4RotationMatrix;
        rot_y->rotateY(-1.0*rotation*rad);
  ...
  fSelectPrim = true;
  //G4cout << "  calTubs, select_prim: " << fSelectPrim << G4endl;
...
//_____________________________________________________________________________
G4bool calTubs::ProcessHits(G4Step *step, G4TouchableHistory*) {
...
//  G4cout<<" Hits X: "<<fHx<<" Hits Y: "<<fHy<<" Hits Z: "<<fHz<<G4endl;
  //G4cout << "calTubs::ProcessHits: " << track->GetParentID() << G4endl;
...
//_____________________________________________________________________________
void calTubs::CreateOutput(TTree *tree) {
