...
// ********************************************************************
//
// G4EllipticalTube_new implementation
//
...
G4EllipticalTube_new::G4EllipticalTube_new( const G4String &name,
                                          G4double Dx,
                                          G4double Dy,
                                          G4double Dz,
	       				  G4double ScaleF)
  : G4VSolid(name), fDx(Dx), fDy(Dy), fDz(Dz),scale_f(ScaleF)
{
  CheckParameters();
}
...
G4EllipticalTube_new::G4EllipticalTube_new( __void__& a )
  : G4VSolid(a), halfTolerance(0.), fDx(0.), fDy(0.), fDz(0.),scale_f(0),
...
G4EllipticalTube_new::~G4EllipticalTube_new()
{
...
G4EllipticalTube_new::G4EllipticalTube_new(const G4EllipticalTube_new& rhs)
  : G4VSolid(rhs), halfTolerance(rhs.halfTolerance),
    fDx(rhs.fDx), fDy(rhs.fDy), fDz(rhs.fDz),scale_f(rhs.scale_f),
...
G4EllipticalTube_new& G4EllipticalTube_new::operator = (const G4EllipticalTube_new& rhs) 
{
...
void G4EllipticalTube_new::CheckParameters()
{
  // Check dimensions
  //
  ...
  if (fDx < dmin || fDy < dmin || fDz < dmin)
  {
    ...
    message << "Invalid (too small or negative) dimensions for Solid: "
            ...
            << "\n  Dy = " << fDy
            << "\n  Dz = " << fDz;
    G4Exception("G4EllipticalTube_new::CheckParameters()", "GeomSolids0002",
...
void G4EllipticalTube_new::BoundingLimits( G4ThreeVector& pMin,
                                       G4ThreeVector& pMax ) const
{
  pMin.set(-fDx*scale_f,-fDy,-fDz);
  pMax.set( fDx*scale_f, fDy, fDz);
}
...
G4bool
G4EllipticalTube_new::CalculateExtent( const EAxis pAxis,
...
EInside G4EllipticalTube_new::Inside( const G4ThreeVector& p ) const
{
  G4double x = p.x() * fSx * (1.0 + (scale_f - 1.0) * (p.z()+fDz)/(2.0*fDz));
...
G4ThreeVector G4EllipticalTube_new::SurfaceNormal( const G4ThreeVector& p ) const
{
  ...
  else
  {
    ...
    G4Exception("G4EllipticalTube_new::SurfaceNormal(p)", "GeomSolids1002",
                JustWarning, message );
    DumpInfo();
#endif
...
G4ThreeVector
G4EllipticalTube_new::ApproxSurfaceNormal( const G4ThreeVector& p ) const
{
  G4double x = p.x() * fSx * (1.0 + (scale_f - 1.0) * (p.z()+fDz)/(2.0*fDz));
  G4double y = p.y() * fSy;
  G4double distR = fQ1 * (x * x + y * y) - fQ2;
...
G4double G4EllipticalTube_new::DistanceToIn( const G4ThreeVector& p,
                                         const G4ThreeVector& v ) const
{
  ...
  //
  G4double px = pcur.x() * fSx * (1.0 + (scale_f - 1.0) * (p.z()+fDz)/(2.0*fDz));
  G4cout<<" test : "<<scale_f<<"p.z()="<<p.z()<<" "<<(1.0 + (scale_f - 1.0) * (p.z()+fDz)/(2.0*fDz))<<G4endl;
  G4double py = pcur.y() * fSy;
  G4double pz = pcur.z();
  G4double vx = v.x() * fSx * (1.0 + (scale_f - 1.0) * (p.z()+fDz)/(2.0*fDz));
  G4double vy = v.y() * fSy;
  G4double vz = v.z();
...
G4double G4EllipticalTube_new::DistanceToIn( const G4ThreeVector& p ) const
{
...
G4double G4EllipticalTube_new::DistanceToOut( const G4ThreeVector& p,
                                                ...
                                                G4ThreeVector* n ) const
{
  ...
  // Scale elliptical tube to cylinder
  //
  G4double px = p.x() * fSx * (1.0 + (scale_f - 1.0) * (p.z()+fDz)/(2.0*fDz));
  G4double py = p.y() * fSy;
  G4double vx = v.x() * fSx * (1.0 + (scale_f - 1.0) * (p.z()+fDz)/(2.0*fDz));
  G4double vy = v.y() * fSy;
  ...
  if (std::max(distZ, distR) > halfTolerance)
  {
    ...
    message << "Direction: " << v;
    G4cout.precision(oldprc);
    G4Exception("G4EllipticalTube_new::DistanceToOut(p,v)", "GeomSolids1002",
                JustWarning, message );
    DumpInfo();
#endif
    if (calcNorm)
...
G4double G4EllipticalTube_new::DistanceToOut( const G4ThreeVector& p ) const
{
#ifdef G4SPECDEBUG
...
G4GeometryType G4EllipticalTube_new::GetEntityType() const
{
  return G4String("G4EllipticalTube_new");
}
...
G4VSolid* G4EllipticalTube_new::Clone() const
{
  return new G4EllipticalTube_new(*this);
}
...
G4double G4EllipticalTube_new::GetCubicVolume()
{
  if (fCubicVolume == 0.)
  {
...
G4double G4EllipticalTube_new::GetCachedSurfaceArea() const
{
...
G4double G4EllipticalTube_new::GetSurfaceArea()
{
  if(fSurfaceArea == 0.)
  {
...
std::ostream& G4EllipticalTube_new::StreamInfo(std::ostream& os) const
{
  G4int oldprc = os.precision(16);
  os << "-----------------------------------------------------------\n"
     << "    *** Dump for solid - " << GetName() << " ***\n"
     << "    ===================================================\n"
     << " Solid type: G4EllipticalTube_new\n"
     << " Parameters: \n"
     << "    length Z: " << fDz/mm << " mm \n"
...
G4ThreeVector G4EllipticalTube_new::GetPointOnSurface() const
{
...
G4Polyhedron* G4EllipticalTube_new::CreatePolyhedron() const
{
...
G4Polyhedron* G4EllipticalTube_new::GetPolyhedron () const
{
...
void G4EllipticalTube_new::DescribeYourselfTo( G4VGraphicsScene& scene ) const
{
  scene.AddSolid (*this);
}
...
G4VisExtent G4EllipticalTube_new::GetExtent() const
{
