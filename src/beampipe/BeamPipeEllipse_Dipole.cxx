...
//_____________________________________________________________________________
BeamPipeEllipse_Dipole::BeamPipeEllipse_Dipole(G4String nam, G4double xpos,G4double ypos,G4double zpos,G4double length, G4double Dx,G4double Dy, G4double pipeThick, G4double angle,G4double bfield,G4LogicalVolume *top):Detector(), G4VSensitiveDetector(nam), fNam(nam) {
	...
	G4EllipticalTube * Cell_Inner = new G4EllipticalTube( "Cell_Inner", 0.5*Dx, 0.5*Dy, 0.5*length+0.001*meter);

	G4EllipticalTube * shape_Bfield = new G4EllipticalTube( "shape_Bfield", 0.5*Dx-0.001*meter, 0.5*Dy-0.001*meter, 0.5*length-0.001*meter);
...
}//BeamPipeEllipse_Dipole

//_____________________________________________________________________________
G4bool BeamPipeEllipse_Dipole::ProcessHits(G4Step *step, G4TouchableHistory*) {
	...
	//G4cout << "BeamPipeEllipse_Dipole::ProcessHits: " << track->GetParentID() << G4endl;
...
//_____________________________________________________________________________
void BeamPipeEllipse_Dipole::CreateOutput(TTree *tree) {

	//output from BeamPipeEllipse_Dipole
...
//_____________________________________________________________________________
void BeamPipeEllipse_Dipole::ClearEvent() {
	ntrack = 0;
	for(int i=0;i<10;i++){
		fIsHit[i] = kFALSE;
