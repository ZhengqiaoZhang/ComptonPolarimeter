...
//local classes
#include "beampipe/BeamPipeTubs_Quad.h"
#include "DetUtils.h"
using namespace std;

//_____________________________________________________________________________
BeamPipeTubs_Quad::BeamPipeTubs_Quad(G4String nam, G4double xpos,G4double ypos,G4double zpos,G4double length,G4double gradient, G4double pipeSize,G4double pipeThick,  G4double angle,G4LogicalVolume *top):Detector(), G4VSensitiveDetector(nam), fNam(nam) {

	G4RotationMatrix* yRot = new G4RotationMatrix;
	yRot->rotateY(-1.0*angle*rad);
	G4Tubs *pipe2 = new G4Tubs(fNam,pipeSize,pipeSize+pipeThick,length/2,0,3.1415926536*2);
	G4Tubs *shape_Bfield = new G4Tubs("tubs_field",0,pipeSize-0.00001*meter,length/2,0,3.1415926536*2);
	...
	G4LogicalVolume *vol_logic = new G4LogicalVolume(pipe2,mat,fNam);
	 ...
	 G4QuadrupoleMagField *field = new G4QuadrupoleMagField(gradient, G4ThreeVector(xpos, ypos, zpos),yRot);
		...
		vol_inner->SetFieldManager(fman, true);
		 ...
		 new G4PVPlacement(yRot, G4ThreeVector(xpos,0*meter, zpos), vol_inner,"Bfield_inner", top, false, 0);
...
//_____________________________________________________________________________
G4bool BeamPipeTubs_Quad::ProcessHits(G4Step *step, G4TouchableHistory*) {
	...
	//G4cout << "BeamPipeTubs_Quad::ProcessHits: " << track->GetParentID() << G4endl;
...
//_____________________________________________________________________________
void BeamPipeTubs_Quad::CreateOutput(TTree *tree) {

	//output from BeamPipeTubs_Quad
...
//_____________________________________________________________________________
void BeamPipeTubs_Quad::ClearEvent() {
	ntrack = 0;
	for(int i=0;i<10;i++){
		fIsHit[i] = kFALSE;
