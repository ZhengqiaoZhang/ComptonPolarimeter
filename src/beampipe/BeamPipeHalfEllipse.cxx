...
//_____________________________________________________________________________
BeamPipeHalfEllipse::BeamPipeHalfEllipse(G4String nam, G4String LorR,G4double xpos,G4double ypos,G4double zpos,G4double length, G4double Dx,G4double Dy, G4double pipeThick, G4double angle,G4LogicalVolume *top):Detector(), G4VSensitiveDetector(nam), fNam(nam) {
	G4double lrno;
	if(LorR=="Right")lrno=-1.0;
	if(LorR=="Left")lrno=1.0;
	G4RotationMatrix* yRot = new G4RotationMatrix;
	yRot->rotateY(-1.0*angle*rad);
	G4EllipticalTube * Cell_Outer = new G4EllipticalTube( "Cell_Outer", 0.5*Dx+pipeThick, 0.5*Dy+pipeThick, 0.5*length-1e-4*meter);
	G4EllipticalTube * Cell_Inner = new G4EllipticalTube( "Cell_Inner", 0.5*Dx, Dy*0.5, 0.5*length);

	G4SubtractionSolid *shape_vessel_1 = new G4SubtractionSolid(fNam+"_PipeEllipse",Cell_Outer, Cell_Inner);
...
//_____________________________________________________________________________
G4bool BeamPipeHalfEllipse::ProcessHits(G4Step *step, G4TouchableHistory*) {
	...
	//G4cout << "BeamPipeHalfEllipse::ProcessHits: " << track->GetParentID() << G4endl;
...
//_____________________________________________________________________________
void BeamPipeHalfEllipse::CreateOutput(TTree *tree) {

	//output from BeamPipeHalfEllipse
...
//_____________________________________________________________________________
void BeamPipeHalfEllipse::ClearEvent() {
	ntrack = 0;
	for(int i=0;i<10;i++){
		fIsHit[i] = kFALSE;
