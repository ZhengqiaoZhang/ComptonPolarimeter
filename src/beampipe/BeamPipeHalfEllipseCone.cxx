...
//_____________________________________________________________________________
BeamPipeHalfEllipseCone::BeamPipeHalfEllipseCone(G4String nam, G4String LorR, G4double xpos,G4double ypos,G4double zpos,G4double length, G4double Dx,G4double Dy,G4double Dx_top,G4double Dy_top, G4double pipeThick, G4double angle,G4LogicalVolume *top):Detector(), G4VSensitiveDetector(nam), fNam(nam) {
	...
	G4EllipticalCone * Cell_Outer = new G4EllipticalCone( "Cell_Outer", pxSemiAxis_Outer,pySemiAxis_Outer,zMax_Outer,0.5*length-0.0001*meter);
	G4EllipticalCone * Cell_Inner = new G4EllipticalCone( "Cell_Inner", pxSemiAxis_Inner,pySemiAxis_Inner,zMax_Innter,0.5*length);

	G4SubtractionSolid *shape_vessel_1 = new G4SubtractionSolid(fNam+"_PipeEllipseCone",Cell_Outer, Cell_Inner);

	G4Box *cutoff = new G4Box("box_cut",Dx,Dy,0.6*length);
        G4ThreeVector subtractBoxPosition(lrno*Dx, 0, 0);
        G4Transform3D subtractBoxTransform(G4RotationMatrix(), subtractBoxPosition);

	G4SubtractionSolid* shape_vessel = new G4SubtractionSolid("subtractedSolid", shape_vessel_1,cutoff, subtractBoxTransform);
...
}//BeamPipeHalfEllipseCone

//_____________________________________________________________________________
G4bool BeamPipeHalfEllipseCone::ProcessHits(G4Step *step, G4TouchableHistory*) {
	...
	//G4cout << "BeamPipeHalfEllipseCone::ProcessHits: " << track->GetParentID() << G4endl;
...
//_____________________________________________________________________________
void BeamPipeHalfEllipseCone::CreateOutput(TTree *tree) {

	//output from BeamPipeHalfEllipseCone
...
//_____________________________________________________________________________
void BeamPipeHalfEllipseCone::ClearEvent() {
	ntrack = 0;
	for(int i=0;i<10;i++){
		fIsHit[i] = kFALSE;
