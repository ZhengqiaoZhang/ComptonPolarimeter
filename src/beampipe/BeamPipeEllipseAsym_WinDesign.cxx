...
//_____________________________________________________________________________
BeamPipeEllipseAsym_WinDesign::BeamPipeEllipseAsym_WinDesign(G4String nam, G4String LorR,G4double xpos,G4double ypos,G4double zpos,G4double length, G4double Dx_t,G4double Dy_t,G4double Dx_b,G4double Dy_b, G4double pipeThick, G4double angle,G4LogicalVolume *top):Detector(), G4VSensitiveDetector(nam), fNam(nam) {
	...
	G4ThreeVector subtractBoxPosition(-0.045069720*meter, 0, -1.0141990*meter);
	...
	if(LorR!="All"){
		...
		G4Box *cutoff_1 = new G4Box(fNam, 2*meter/2., 2*meter/2., 10*meter/2.);
		G4ThreeVector subtractBoxPosition_1(lrno*1*meter, 0, 0);
		G4Transform3D subtractBoxTransform_1(G4RotationMatrix(), subtractBoxPosition_1);

		G4SubtractionSolid* shape_vessel_1 = new G4SubtractionSolid("subtractedSolid", ellipticalTube,cutoff_1, subtractBoxTransform_1);
...
}//BeamPipeEllipseAsym_WinDesign

//_____________________________________________________________________________
G4bool BeamPipeEllipseAsym_WinDesign::ProcessHits(G4Step *step, G4TouchableHistory*) {
	...
	//G4cout << "BeamPipeEllipseAsym_WinDesign::ProcessHits: " << track->GetParentID() << G4endl;
...
//_____________________________________________________________________________
void BeamPipeEllipseAsym_WinDesign::CreateOutput(TTree *tree) {

	//output from BeamPipeEllipseAsym_WinDesign
...
//_____________________________________________________________________________
void BeamPipeEllipseAsym_WinDesign::ClearEvent() {
	ntrack = 0;
	for(int i=0;i<10;i++){
		fIsHit[i] = kFALSE;
