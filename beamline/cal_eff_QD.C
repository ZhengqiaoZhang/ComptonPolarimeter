...
void cal_eff_QD(){
	...
	TFile *store = new TFile("result_QD_S3.root","OPEN");
	TH2D *xz = (TH2D*)store->Get("x_z");
	TFile *store_S1 = new TFile("result_QD_S1.root","OPEN");
...
/*	TF1 *f1 = new TF1("myfunc",fBX,0,2000,1);
	...
	for(int i=1;i<200;i++){
		...
		double xvalue = 0;
		double x_accept = xvalue - sqrt(BX(14.234+zvalue*0.01)*24.0*1.0e-9)*13.0*100;
		...
		for(int j=1;j<2001;j++){
			totale=totale+xz->GetBinContent(i,j);
				...
				cout<<zvalue<<" "<<eAccept*1.0/(totale*1.0)<<endl;
		z_gap[i-1]=zvalue;
