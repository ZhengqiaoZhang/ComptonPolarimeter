...
void draw_cone_e(){
	...
	TFile *store = new TFile("result_S3.root","OPEN");
	TH2D *xz = (TH2D*)store->Get("x_z");
	TFile *store_S1 = new TFile("result_S1.root","OPEN");
	TH2D *xz_S1 = (TH2D*)store_S1->Get("x_z");
	double eff_S1[1999], eff_S3[1999],z_gap[1999];
...
/*	 TF1 *f1 = new TF1("myfunc",fBX,0,2000,1);
	...
	for(int i=1;i<2000;i++){
		double zvalue = i*1.0;
		double xvalue = zvalue*tan(4.38602*1.0e-3);
		double x_accept = xvalue - sqrt(BX(13.334+zvalue*0.01)*24.0*1.0e-9)*13.0*100;
		int Xbin = floor((x_accept+10.0)/0.01);
		int eAccept = 0;
	...
	TGraph* g_S1 = new TGraph(1999,z_gap,eff_S1);
	TGraph* g_S3 = new TGraph(1999,z_gap,eff_S3);
